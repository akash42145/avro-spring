...
public class CarSerDe {
  ...
  public List<Car> deserializeList(List<byte[]> bytes) throws SerializationException {
	  return bytes.stream().map(e -> deserializer.deserialize(Car.class, e)).collect(Collectors.toList());
  ...
  public List<byte[]> serializeList(List<Car> car) throws SerializationException {
	  
	 List<byte[]> bytes =car.stream().map(e -> serializer.serialize(e) ).collect(Collectors.toList());
	    return bytes;
	  }
}
